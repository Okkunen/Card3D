...
- Use single-line comments with `#` for all documentation and explanations
- Do NOT use triple-quote docstrings (`"""..."""`) - this is Python style, not GDScript convention
- For multi-line comments, use multiple `#` lines
- Example:
  ```gdscript
  # Calculate the damage dealt to the target character.
  # Takes into account strength, armor, and critical hits.
  func calculate_damage(attacker: Character, target: Character) -> int:
  ```
...
4. @export variables
5. Regular var declarations (public)
6. Regular var declarations (private, prefixed with _)
7. @onready variables
